...
Algoritmos como o Counting Sort e o Radix Sort superam o limite inferior de Ω(n \log n) característico dos métodos baseados em comparação porque:
...
- **C)** Θ(n \log k) no pior caso e Θ(n) no melhor caso.
...
- **D)** Θ(n + k), necessitando de um vetor de frequências de tamanho k+1 e um vetor de saída de tamanho n.
...
- **A)** Empregar o Heapsort clássico, obtendo complexidade de tempo Θ(n \log n).
- **B)** Empregar o Radix Sort representando os números na base n, resultando em d=2 dígitos e complexidade linear Θ(n).
- **C)** Empregar o Counting Sort diretamente no intervalo [0, n^2 - 1], garantindo tempo Θ(n).
- **D)** Empregar o Quick Sort com pivô no primeiro elemento, o qual sempre opera em tempo linear.
- **E)** Nenhum método algorítmico pode ordenar tal conjunto em tempo inferior a Ω(n \log n) por restrição da árvore de decisão.
...
1. O Quick Sort clássico apresenta complexidade de tempo média igual a Θ(n \log n) e pior caso O(n^2).
2. O Counting Sort executa sempre em tempo estritamente proporcional a Θ(n), independentemente da amplitude do intervalo k dos valores da entrada.
...
| **04** | **B** | Complexidade assintótica de tempo Θ(n + k). |
| **05** | **C** | Degeneração por amplitude k \gg n no Counting Sort. |
| **06** | **D** | Espaço auxiliar Θ(n + k) para vetores C e B. |
...
| **23** | **B** | Tempo linear Θ(n) para chaves de tamanho fixo em bits. |
...
* **Questão 01 (C):** Em qualquer modelo de árvore de decisão binária para ordenação por comparações, as folhas representam as permutações possíveis das entradas (n!). A altura da árvore corresponde ao número de comparações no pior caso, sendo no mínimo \lceil \log_2(n!) \rceil \approx n \log_2 n - n \log_2 e = Ω(n \log n).
* **Questão 02 (C):** O Counting Sort e o Radix Sort escapam da cota Ω(n \log n) porque calculam posições de escrita avaliando a representação aritmética dos dados e utilizando-as como índices em vetores auxiliares, sem submeter os pares à operação A[i] < A[j].
...
* **Questão 04 (B):** O Counting Sort executa laços sequenciais predeterminados: inicialização do vetor de contagem (Θ(k)), contagem de ocorrências (Θ(n)), soma prefixada (Θ(k)) e distribuição de saída (Θ(n)). A ordem prévia dos dados não altera o número de passos, resultando em Θ(n + k) para todos os casos.
* **Questão 05 (C):** Quando k = 10^9 e n = 1.000, o vetor de contagem exigirá a inicialização e percurso de 10^9 posições de memória, operando em Θ(10^9) passos. Em contrapartida, algoritmos O(n \log n) realizam da ordem de 1.000 \cdot \log_2(1000) \approx 10^4 operações.
* **Questão 06 (D):** Para garantir estabilidade e calcular os endereços, o algoritmo instancia um vetor de frequências acumuladas C com k+1 posições e um vetor de saída B com n posições, perfazendo espaço auxiliar Θ(n + k).
* **Questão 07 (A):**
...
* **Questão 10 (B):** A execução envolve d iterações independentes de uma rotina estável (Counting Sort). Como cada etapa de contagem sobre uma base b consome Θ(n + b), o tempo global é Θ(d \cdot (n + b)).
* **Questão 11 (B):** Com valores limitados a n^2 - 1, ao adotar a base b = n, qualquer elemento pode ser expresso por no máximo d = \log_n(n^2) = 2 dígitos. A complexidade do Radix Sort passa a ser Θ(2 \cdot (n + n)) = Θ(n), que é estritamente linear.
...
* **Questão 20 (C):** A afirmativa 1 é correta (Quick Sort clássico tem caso médio Θ(n \log n) e pior caso O(n^2)). A afirmativa 2 é falsa, pois se k = Ω(n^2), o tempo do Counting Sort se torna quadrático. A afirmativa 3 é verdadeira, pois a instabilidade da sub-rotina desordena os dígitos menos significativos.
...
* **Questão 23 (B):** A complexidade do Radix Sort é Θ(d \cdot (n + b)). Com base fixa b = 2 e quantidade constante de dígitos d = 64, obtém-se Θ(64 \cdot (n + 2)) = Θ(n), que é tempo linear em relação à quantidade de elementos n.
* **Questão 24 (A):**
